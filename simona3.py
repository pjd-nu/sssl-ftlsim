...
for i in range(nelements):
	...
	s.elem = i
	s.blkno = j
    initblks.append(list)
    physblks.append(copy.copy(list))
...
def putblk(elmt, blk):
    ...
    #if print_putblk:
    #    print "putblk:", blk.blkno, elmt, idx
    assert _map[idx] is None
    _map[idx] = blk
    blks_free[elmt] += 1

for i in range(nelements):
    elements[i].add_segment(initblks[i].pop(0))
...
for lba in range(U*Np):
    ...
    e.i += 1
    if e.i >= Np:
        e.add_segment(initblks[elem].pop(0))
